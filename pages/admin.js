...
export default function Admin() {
  ...
  const loadDates = async () => {
    ...
    (data.dates || []).forEach(d => {
      config[d.date] = { slots: d.slots ?? 0, reason: d.reason };
    });
  ...
  const saveDate = async (date, slots) => {
    await fetch("/api/admin-dates", {
      ...
      body: JSON.stringify({ date, slots, reason: slots === 0 ? "Bloqué" : `${slots} commandes max` }),
    });
  };
  ...
  const toggleDay = (dateStr, isPast) => {
    if (isPast) return;
  ...
  const getDayStyle = (dateStr, isPast, isSelected) => {
    if (isSelected) return { bg: "#2a1810", color: "#fff", border: "#2a1810" };
  ...
  const selectAllMonth = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      if (dateObj >= today) dates.push(dateStr);
    }
  ...
  const blockedList = Object.entries(datesConfig).filter(([,v]) => v.slots === 0).sort(([a],[b]) => a.localeCompare(b));
  const limitedList = Object.entries(datesConfig).filter(([,v]) => v.slots > 0).sort(([a],[b]) => a.localeCompare(b));
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.container}>
        ...
        {/* Instructions */}
        <div style={S.infoBox}>
          💡 <strong>Clique sur les jours</strong> pour les sélectionner (plusieurs à la fois), puis choisis le nombre de commandes autorisées.
        </div>

        {/* Légende */}
        <div style={S.legend}>
          <div style={S.legendItem}><div style={{...S.legendDot, background:"#2a1810"}}/> Sélectionné</div>
        ...
        <div style={S.card}>
          ...
          <div style={{display:"flex", gap:8, marginBottom:16, flexWrap:"wrap"}}>
            <button style={S.quickBtn} onClick={selectAllMonth}>Sélectionner tout le mois</button>
            {selectedDays.length > 0 && (
              <button style={S.quickBtn} onClick={clearSelection}>Désélectionner ({selectedDays.length})</button>
            )}
          </div>
          ...
          <div style={S.calGrid}>
            ...
            {Array.from({length: daysInMonth}).map((_, i) => {
              ...
              const colors = getDayStyle(dateStr, isPast, isSelected);
              const cfg = datesConfig[dateStr];
              const status = getDayStatus(dateStr);

              return (
                <div key={day} onClick={() => toggleDay(dateStr, isPast)} style={{
                  ...S.dayCell,
                  ...
                  {!isPast && status === "limited" && !isSelected && (
                    <div style={{fontSize:9, color:"#b07020"}}>{cfg.slots} max</div>
                  )}
                  {!isPast && status === "blocked" && !isSelected && (
                    <div style={{fontSize:9, color:"#c03030"}}>✕</div>
                  )}
          ...
          {selectedDays.length > 0 && (
            <div style={S.actionBar}>
              <div style={{fontSize:13, color:"#6a5f5a"}}>
                <strong>{selectedDays.length}</strong> jour{selectedDays.length > 1 ? "s" : ""} sélectionné{selectedDays.length > 1 ? "s" : ""}
              </div>
              <button style={S.actionBtn} onClick={() => setShowSlotPicker(true)}>
        ...
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:16}}>
          <div style={S.card}>
            <div style={S.cardTitle}>🚫 Bloqués ({blockedList.length})</div>
            {blockedList.length === 0
              ? <p style={S.hint}>Aucune date bloquée</p>
              : blockedList.map(([date]) => (
                <div key={date} style={S.dateRow}>
                  ...
                  <button style={S.unblockBtn} onClick={() => { unblockDate(date); loadDates(); }}>✕</button>
                </div>
              ))
            }
          </div>
          <div style={S.card}>
            <div style={S.cardTitle}>⚠️ Limités ({limitedList.length})</div>
            {limitedList.length === 0
              ? <p style={S.hint}>Aucune date limitée</p>
              : limitedList.map(([date, cfg]) => (
                <div key={date} style={S.dateRow}>
                  <div style={S.dateText}>
                    {new Date(date + "T12:00:00").toLocaleDateString("fr-FR", {day:"numeric", month:"short"})}
                    <span style={{color:"#b07020", marginLeft:6, fontSize:12}}>{cfg.slots} max</span>
                  </div>
                  <button style={S.unblockBtn} onClick={() => { unblockDate(date); loadDates(); }}>✕</button>
                </div>
              ))
      ...
      {showSlotPicker && (
        <div style={S.overlay}>
          <div style={S.popup}>
            ...
            <div style={S.popupSub}>Combien de commandes max par jour ?</div>
...
const S = {
  ...
  container: { maxWidth:800, width:"100%", padding:"24px 20px", display:"flex", flexDirection:"column", gap:16, margin:"0 auto" },
  infoBox: { background:"#fdf8f4", border:"1px solid #ede5de", borderRadius:6, padding:"12px 16px", fontSize:13, color:"#6a5f5a" },
  ...
  quickBtn: { background:"#faf8f5", border:"1.5px solid #ede5de", borderRadius:4, padding:"6px 12px", cursor:"pointer", fontSize:12, color:"#6a5f5a", fontFamily:"inherit" },
  ...
  dayCell: { borderRadius:6, padding:"8px 4px", textAlign:"center", cursor:"pointer", transition:"all 0.12s", minHeight:52, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:2 },
  actionBar: { display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:16, padding:"12px 16px", background:"#fdf0ee", border:"1px solid #f0c8b8", borderRadius:6 },
  actionBtn: { background:"#c9736a", color:"#fff", border:"none", borderRadius:4, padding:"10px 20px", fontSize:13, cursor:"pointer", fontFamily:"inherit", fontWeight:600 },
  ...
  slotBtn: { padding:"10px 12px", borderRadius:6, cursor:"pointer", fontSize:13, fontFamily:"Georgia, serif", textAlign:"center", transition:"all 0.15s" },
};
